...
import { FaUser, FaEnvelope, FaPhone, FaBuilding, FaMapMarkerAlt, FaGlobe, FaCity, FaHome, FaLock, FaCalendar } from 'react-icons/fa';
      ...
      >
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <DetailItem icon={FaLock} label="User ID" value={userDetails.userId} />
            <DetailItem icon={FaCalendar} label="Expiry Date" value={userDetails.expiryDate} />
            <DetailItem icon={FaEnvelope} label="Email" value={userDetails.email} />
            <DetailItem icon={FaPhone} label="Mobile No." value={userDetails.mobile} />
            <DetailItem icon={FaBuilding} label="Company" value={userDetails.company} />
            <DetailItem icon={FaHome} label="Address" value={userDetails.address} />
            <DetailItem icon={FaCity} label="City" value={userDetails.city} />
            <DetailItem icon={FaMapMarkerAlt} label="State" value={userDetails.state} />
            <DetailItem icon={FaGlobe} label="Country" value={userDetails.country} />
            <DetailItem icon={FaLock} label="Pincode" value={userDetails.pincode} />
          </div>

          {/* Status Section */}
          <div className="flex items-center gap-4 border-t pt-4">
            <span className="font-medium text-gray-700">Status:</span>
            <span className={`px-3 py-1 rounded-full text-sm font-semibold ${userDetails.status === 'enable' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
              {userDetails.status === 'enable' ? 'Enabled' : 'Disabled'}
            </span>
          </div>
        </div>
      </Dialog>
