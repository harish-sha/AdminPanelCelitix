...
import { getWhatsappFlow, getWabaList, getWhatsappFlowTemplate } from "@/apis/whatsapp/whatsapp";
  ...
  return (
    <>
      ...
      <div className=" bg-white border border-gray-300 rounded-xl shadow-sm">
        <div className="p-4">
          <div className="flex flex-col sm:flex-row sm:justify-between items-center mb-4">
            <h2 className="flex items-center gap-2 text-xl font-semibold mb-2 sm:mb-0">Created Flows
              <FaWhatsapp className="text-[#25D366] text-2xl" />
            </h2>
            ...
            {paginatedFlows.map((flow, index) => (
              ...
              >
                ...
                <div className="text-sm text-center min-w-[80px]">
                  <div className="font-semibold text-sm mb-2">Flow Category</div>
            ...
            <form onSubmit={handleSubmitFlowTemp}>
              {Array.isArray(selectedFlowDetails)
                ? selectedFlowDetails.map((flow, idx, name) => (
                  <div className='flex flex-col gap-4' key={flow.flowId || idx}>
                    <span><strong>Flow Name:</strong>{flow.flowName}</span>
                  </div>
                ))
                : selectedFlowDetails && (
                  <div className='flex flex-col gap-4'>
                    <div className="flex flex-col justify-start  items-start bg-gray-800 min-h-55 rounded-xl ">
                      <div className="flex flex-row items-center gap-2 mt-2 border-b w-full py-2 px-2 border-gray-500">
                        <img src="https://static.vecteezy.com/system/resources/previews/048/216/750/original/cartoon-man-avatar-character-male-avatar-profile-free-png.png" className="w-10 h-10 border border-gray-600 rounded-full" />
                        <div className="">
                          <div className=" text-md text-gray-50">
                            {selectedWaba
                              ? (wabaList.find(waba => waba.mobileNo === selectedWaba)?.name || '')
                              : ''}
                          </div>
                        </div>
                      </div>
                      <div className="p-5">
                        <div className="bg-gray-700 rounded-t-xl rounded-br-xl p-3 text-white w-60 shadow ">
                          <div className="mb-2 text-sm break-words text-wrap">{bodyText}</div>
                          <div className="flex items-center justify-between border-t border-gray-500 pt-2 mt-2 break-words text-wrap">
                            ...
                            >
                              <svg width="20" height="20" fill="none" viewBox="0 0 24 24">
                                ...
                                <path d="M8 8h8v2H8V8zm0 4h8v2H8v-2z" fill="#757575" />
                              </svg>
                              <div className="break-words text-wrap" >
                                <div className="font-medium text-sm break-words text-wrap">{btnText}</div>
                              </div>
                            </button>
                          </div>
                        </div>
                    ...
                    <span className="flex justify-center items-center">Flow :&nbsp;<strong>{selectedFlowDetails.flowName}</strong></span>
                    <AnimatedDropdown
