...
export default function PreviewDrawer({
  ...
  const name = details?.waba?.find((p) => p.mobileNo == details?.selected)?.name
      ...
      <>
        <div className=" bg-[#ece5dd] p-3 rounded-md w-fit relative">
          <div className="bg-white rounded-xl shadow-md p-4 w-[300px]">
            {/* Header text */}
            <p className="text-sm text-black font-semibold mb-1">{node?.text || "No Heading"}</p>
            {/* Body text */}
            <p className="text-sm break-words whitespace-pre-wrap text-wrap text-black mb-3 mt-4">{node?.message || "No Body"}</p>
            {/* Footer */}
            ...
            {/* Button */}
            <div className="text-green-600 flex items-center justify-center space-x-1 cursor-pointer text-sm relative" onClick={toggleDrawer}>
              <ListIcon size={14} />
              ...
              <div className=" bg-[#ece5dd] p-3 rounded-md w-full ">
                <div className="bg-white rounded-xl shadow-md p-4 ">
                  <div className='flex flex-col justify-center items-center mb-6'>
                    <p className="text-sm text-black font-semibold mb-1">{node?.text}</p>
                  </div>
                  {node?.options?.map((section, index) => (
                    <div className="relative" >
                      <div key={index} className='flex flex-col justify-start items-start border-t border-gray-200 mt-3 w-full'>
                        <p className='text-sm text-black font-medium mt-2 break-words text-wrap w-full'>{section?.option} </p>
                        <span className='text-xs text-gray-500 font-medium break-words text-wrap w-full'>{section?.value} </span>
                      </div>
                      <div className='absolute top-5 right-5 w-4 h-4 border border-gray-400 rounded-full' >
    ...
    return (
      <div className=" bg-[#ece5dd] p-3 rounded-md w-fit">
        <div className="bg-white rounded-xl shadow-md p-4 w-[300px] ">
          ...
          <div className=''>
            <p className='text-sm text-wrap break-words text-black'>{node?.message || "No Body"} </p>
          </div>

          <div className=''>
            <p className='text-xs text-wrap break-words text-gray-400 mt-2'>{node?.buttonFooter || "No Footer"}</p>
          </div>
          ...
          <div className='flex flex-col justify-center items-center gap-4'>
            ...
            {node?.buttonTexts?.map((button, index) => (
              <div key={index} >
                <div className=''>
                  <ReplyOutlinedIcon className="text-green-600" /> <span className="text-green-600 text-sm">{button}</span>
          ...
          <div className="bg-white rounded-xl shadow-md max-w-md w-full overflow-hidden mr-22">
            ...
            <div className="p-2">
              <div className="flex flex-col gap-2 text-xs text-gray-500">
                <span className='text-black font-semibold'>{node?.fileCaption || ""}</span>
            ...
            <div className="p-3 overflow-auto flex-1">
              <div className="h-full w-full flex flex-col bg-[#ECE5DD] rounded-md overflow-hidden border">
                ...
                <div className="flex items-center justify-between bg-[#075E54] text-white h-12 px-3">
                  ...
                  <span className="text-xl leading-none cursor-pointer" onClick={onClose}>×</span>
                </div>
