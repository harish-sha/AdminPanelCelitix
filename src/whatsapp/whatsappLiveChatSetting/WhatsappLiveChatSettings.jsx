...
import { Dialog } from "primereact/dialog";
import toast from "react-hot-toast";
import { BiMessageSquareDetail, BiCog, BiUserCircle, BiChevronRight } from 'react-icons/bi';
...
const tabs = [
  { key: 'configuration', label: 'AI Configurations', icon: <BiMessageSquareDetail size={20} /> },
      ...
      <>
        ...
        >
          ...
          <div className="bg-white rounded-2xl shadow-lg flex flex-col items-center justify-center h-auto p-5 border-3 border-dashed border-indigo-200">
            {wabaState.selected ? (<div className=" flex flex-wrap justify-center items-center gap-5 space-y-8 mx-auto">
              {liveChatCards.map((card) => {
                return (
                  <>
                    <div
          ...
          {tabs.map((tab) => (
            <div key={tab.key}>
              <button
                ...
                className={`flex items-center w-full px-4 py-3 mb-1 text-sm font-medium rounded-lg transition-colors hover:bg-gray-100 focus:outline-none ${activeTab === tab.key ? 'bg-blue-100 text-blue-600' : 'text-gray-700'
                  }`}
              >
